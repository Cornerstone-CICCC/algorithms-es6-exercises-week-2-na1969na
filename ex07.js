...
const checkAir = function (samples, threshold) {
  const dirtySamples = samples.reduce(
    (acc, cur) => (cur === "dirty" ? acc + 1 : acc),
    0
  );

  const pollutionRatio = dirtySamples / samples.length;

  return pollutionRatio > threshold ? "Polluted" : "Clean";
};
